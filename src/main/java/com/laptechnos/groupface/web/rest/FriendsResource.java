...
/**
 * REST controller for managing {@link com.laptechnos.groupface.domain.Friends}.
 */
@RestController
@RequestMapping("/api")
public class FriendsResource {
    ...
    public ResponseEntity<List<FriendsDTO>> getAllFriends(
        ...
        Page<FriendsDTO> page;
        if (eagerload) {
            page = friendsService.findAllWithEagerRelationships(pageable);
        } else {
            page = friendsService.findAll(pageable);
        }
    ...
    /**
     * {@code SEARCH  /_search/friends?query=:query} : search for the friends corresponding
     * to the query.
     *
    ...
    public ResponseEntity<List<FriendsDTO>> searchFriends(
        @RequestParam String query,
        @org.springdoc.api.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to search for a page of Friends for query {}", query);
        Page<FriendsDTO> page = friendsService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
