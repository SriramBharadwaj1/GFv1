...
interface OrganisationSearchRepositoryInternal {
    ...
    Page<Organisation> search(Query query);

    void index(Organisation entity);
}
...
class OrganisationSearchRepositoryInternalImpl implements OrganisationSearchRepositoryInternal {
    ...
    OrganisationSearchRepositoryInternalImpl(ElasticsearchRestTemplate elasticsearchTemplate, OrganisationRepository repository) {
    ...
    @Override
    public Page<Organisation> search(Query query) {
        SearchHits<Organisation> searchHits = elasticsearchTemplate.search(query, Organisation.class);
        List<Organisation> hits = searchHits.map(SearchHit::getContent).stream().collect(Collectors.toList());
        return new PageImpl<>(hits, query.getPageable(), searchHits.getTotalHits());
    }

    @Override
    public void index(Organisation entity) {
