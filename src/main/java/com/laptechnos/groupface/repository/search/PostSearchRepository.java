...
class PostSearchRepositoryInternalImpl implements PostSearchRepositoryInternal {
    ...
    PostSearchRepositoryInternalImpl(ElasticsearchRestTemplate elasticsearchTemplate, PostRepository repository) {
    ...
    public Page<Post> search(Query query) {
        ...
        List<Post> hits = searchHits.map(SearchHit::getContent).stream().collect(Collectors.toList());
