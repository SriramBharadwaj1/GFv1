...
class CommentSearchRepositoryInternalImpl implements CommentSearchRepositoryInternal {
    ...
    CommentSearchRepositoryInternalImpl(ElasticsearchRestTemplate elasticsearchTemplate, CommentRepository repository) {
    ...
    public Page<Comment> search(Query query) {
        ...
        List<Comment> hits = searchHits.map(SearchHit::getContent).stream().collect(Collectors.toList());
